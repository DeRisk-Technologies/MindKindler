...
export default function RulebookVaultPage() {
    // In real app, filter where type == 'rulebook'
    const { data: docs, loading } = useFirestoreCollection<KnowledgeDocument>("knowledgeDocuments", "createdAt", "desc");
    const [search, setSearch] = useState("");

    const rulebooks = docs.filter(d => d.type === 'rulebook' && d.title.toLowerCase().includes(search.toLowerCase()));

    return (
        <div className="p-8 space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold">Rulebook Vault</h1>
                    <p className="text-muted-foreground">Official compliance documents, laws, and policies.</p>
                </div>
                <DocumentUploadDialog type="rulebook" />
            </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            ...
                            {!loading && rulebooks.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No rulebooks uploaded yet.</TableCell>
                                </TableRow>
                            )}
                            {rulebooks.map(doc => (
                                <TableRow key={doc.id}>
                                    ...
                                    <TableCell>{doc.metadata.authority || "N/A"}</TableCell>
                                    <TableCell>
