// src/services/ai/InterventionPlanner.ts
 ...
 * Bridges the gap between "Clinical Evidence" (Transcript + Scores) and "Approved Programs" (Library).
 */
export class InterventionPlanner {
     ...
     */
    async generatePlan(context: PlanningContext): Promise<PlannedIntervention[]> {
        
        // 1. Prepare Data for Prompt
        const needsSummary = this.summarizeNeeds(context);
        const libraryContext = this.formatLibrary(context.availableInterventions);
    ...
    private summarizeNeeds(context: PlanningContext): string {
    ...
    private formatLibrary(interventions: InterventionLogic[]): string {
